...
def fast_process(
    ...
    original_w, original_h = image_origin.shape[:2]
...
def segment_everything(
    ...
    print("[Info] Truoc resize: ", input.shape)
    ...
    # new_w = int(w * scale)
    # new_h = int(h * scale)
    # input.resize((new_w, new_h), refcheck=False)

    print("[Info] Sau resize: ", input.shape)

    # exit(1)
    fig = fast_process(annotations=results[0].masks.data,
                       image=input,
                       image_origin=input_ori,
                       device=device,
