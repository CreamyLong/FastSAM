...
class FastSam(object):
    ...
    def segment(self, bgr_img):
        ...
        ## Inference
        t1 = time.time()
        print("[pre_processing]: ", inp[0].transpose(0, 1, 2).shape)  # (3, 1024, 1024)
        ...
        print("[postprocess]: ", masks.shape)  #[38, 1920, 1080]


        image_with_masks = segment_everything(input=bgr_img, results=result, input_size=self.imgsz[0])
        print('保存图片')
        print(image_with_masks)
        # cv2.imwrite("./obj_segment_trt.png", image_with_masks)
        image_with_masks.save("./output/obj_segment_trt.png")
        print('保存图片完成')
        return masks


if __name__ == '__main__':
    model = FastSam(model_weights="./weights/fast_sam_1024.trt", max_size=1024)
